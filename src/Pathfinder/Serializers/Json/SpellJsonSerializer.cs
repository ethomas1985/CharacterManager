...
namespace Pathfinder.Serializers.Json
{
    public class SpellJsonSerializer : AbstractJsonSerializer<ISpell>
    {
        ...
        protected override ISpell DeserializeFromJson(JsonSerializer pSerializer, JObject pJobject)
        {
            ...
            var descriptionToken = pJobject.GetValue(nameof(ISpell.Description));
            var isList = descriptionToken.Type == JTokenType.Array;
            var description = isList
                ? GetValuesFromArray<string>(pSerializer, pJobject, nameof(ISpell.Description))
                : new List<string>
                {
                    GetString(pJobject, nameof(ISpell.Description))
                };
            ...
            var levelRequirements =
                GetValuesFromHashObject<string, int>(pSerializer, pJobject, nameof(ISpell.LevelRequirements));
            ...
            return new Spell(name, magicSchool, magicSubSchools, magicDescriptors, savingThrow, description,
                             hasSpellResistance, spellResistance, castingTime, range, levelRequirements, duration,
                             spellComponents);
        }
    }
}
